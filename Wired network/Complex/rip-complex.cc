...
int main (int argc, char **argv){
	...
	NetDeviceContainer netDev4 = p2p.Install (cont4);
	NetDeviceContainer netDev5 = p2p.Install (cont5);
	NetDeviceContainer netDev6 = p2p.Install (cont6);
	NetDeviceContainer netDev7 = p2p.Install (cont7);
	NetDeviceContainer netDev8 = p2p.Install (cont8);
	...
	listRouting.Add (staticRoutingHelper, 5);
	...
	Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("rip-routing-table.txt", std::ios::out);
	...
	p2p.EnableAsciiAll (ascii.CreateFileStream ("rip-trace-topo2.tr"));
	// p2p.EnablePcapAll ("rip-simple-routing", true);
	...
	anim= new AnimationInterface("rip-anim2.xml");
	...
	flowMonitor->SerializeToXmlFile("rip-flow2.xml", true, true);  
